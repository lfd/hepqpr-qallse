...
def _get_shapes(dims):
    if dims == ["x", "y"]:
        return xy_layer_shapes
    if dims == ["z", "r"]:
        return rz_layer_shapes
    return []


def _get_layers_button(dims, xpad=0.1):
...
def _get_toggle_line_button(xpad=0.1):
    return dict(
        type="buttons",
        buttons=[
            dict(
                label="hits+tracks", method="update", args=[{"mode": "markers+lines"}]
            ),
...
def _get_ratio_button(xpad=0.1):
    return dict(
        type="buttons",
        buttons=[
...
def create_trace(hits, t, dims=None, **trace_params):
    ...
    coords = dict(
        (ax, v) for _, ax, v in zip(dims, list("xyz"), hits.loc[t][dims].values.T)
    )
    if len(dims) == 3:
        params = merge_dicts(
            dict(marker=dict(size=3), hoverinfo="name+text", **coords), trace_params
        )
...
def show_plot(
    traces, dims, show_buttons, return_fig=False, filename=None, **layout_params
):
    if dims is None:
        dims = default_dims
    ...
    if len(dims) == 3:  # 3D
        ...
        if len(layout_params) > 0:
            params = merge_dicts(params, layout_params)
        ...
        params = dict(
            showlegend=True, width=800, height=800, hovermode="closest", **ax_titles
        )
        if len(layout_params) > 0:
            params = merge_dicts(params, layout_params)
        layout = go.Layout(**params)
