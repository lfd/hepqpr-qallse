...
_extract_energy = lambda line: float(
    re.match(".*((loop =)|(answer  ))(-?\d+\.\d*)", line).groups()[-1]
)
...
def plot_energies(times, answers, annotations=None, filename=None):
    ...
    title += ("<br>" * 2) + "<br>".join(
        [f"{colors[sol_types[k]]} = <i>{k}</i>" for k in set(annotations)]
    )
...
@click.option(
    ...
    is_flag=True,
    default=True,
    help="Whether or not to generate plots.",
)
def cli(input, output, plot):
    ...
    if output is not None and not output.endswith(".csv"):
        print(
            f'error. output "{output}" should be a path to a csv file, i.e. ending with ".csv".'
        )
    ...
    with open(input) as f:
        ...
        if annotations is not None:
            df = pd.DataFrame(
                np.column_stack([times, answers, annotations]),
                columns=["timestamp", "energy", "soltype"],
            )
        else:
            df = pd.DataFrame(
                np.column_stack([times, answers]), columns=["timestamp", "energy"]
            )
